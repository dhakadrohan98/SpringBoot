...
public interface OrderHeaderNewRepository extends JpaRepository <OrderHeaderNew,String> {
    ...
    @Query(value = "SELECT person.first_name, person.last_name, person.middle_name, party.* " +
            "FROM person INNER JOIN party ON person.party_id = party.party_id " +
            "WHERE party.party_id = :partyId", nativeQuery = true)
    public String findPerson(@Param("partyId") String partyId);
